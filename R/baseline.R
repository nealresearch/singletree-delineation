...
source("//home//nilraj.shrestha//R//umixing//R_script//weighted_polygon.R")
source("//home//nilraj.shrestha//R//umixing//R_script//get_sentinel_polygon.R")
sen_val <- sentinel_df()
...
smooth_one_df <- function(df) {
  df2 <- df[, !(names(df) %in% c("polygon_id"))]
  df_xy <- df[, c("polygon_id")]
...
extract_series <- function(df, x0) {
  row <- df[ df$polygon_id==x0, ]
  if(nrow(row)==0) stop("no data at that point")
  vals <- as.numeric(row[ , -(1)])
  dates <- as.Date(sub(".*_(\\d{4}-\\d{2}-\\d{2})Z$", "\\1", names(row)[-(1)]))
  data.frame(date=dates, value=vals)
}

extract_summer_series <- function(df, x) {
  # 1) grab the row
  row <- df[df$polygon_id == x, , drop = FALSE]
  if (nrow(row) == 0) stop("no data at that point")
...
# --- Function to check if filename is in summer months (June–Sept) ---
is_summer_file <- function(filename) {
  # Remove "Z.tif" and convert to Date
  date_str <- sub("Z\\.tif$", "", filename)
  date <- as.Date(date_str, format = "%Y-%m-%d")
  month_num <- as.numeric(format(date, "%m"))
  return(month_num %in% 6:8)
}
...
for (i in 1:20) {
  ...
  evi_chunks <- foreach(j = seq_along(image_list),
                        .packages = c("raster", "sf", "tibble", "stringr", "purrr", "dplyr"),
                        .export      = c(
...
for (k in seq_along(smoothed_results)){
  smoothed_results[[k]] <- smoothed_results[[k]] |>
    rename(polygon_id = 1)
}
...
end_y <- ncol(smoothed_results[[1]])
xy <- random_number(1, 1, end_y)
poly_id <- xy[1]

x <- smooth_df[["polygon_id"]][1064] #134 17

series_list <- lapply(smoothed_results, extract_series, x=x)
...
sen_row <- sen_val[sen_val$polygon_id == x, , drop = FALSE]
sen_row <- sen_row[,-(1:2), drop=FALSE]
sen_row <- as.numeric(sen_row)
coln <- names(sen_val[2:132])

summer_i <- sapply(coln, is_summer_file)
summer_idx <- unname(summer_i)
...
i <- 1
while (i <= (n - 2)) {
   if (sen_row[i] <= lower[i] && summer_idx[i] == TRUE &&
       sen_row[i + 1] <= lower[i + 1] && summer_idx[i + 1] == TRUE &&
       sen_row[i + 2] <= lower[i + 2] && summer_idx[i + 2] == TRUE) {

     red_flag_full[i]     <- sen_row[i]
     red_flag_full[i + 1] <- sen_row[i + 1]
     red_flag_full[i + 2] <- sen_row[i + 2]
...
band_col <- adjustcolor("blue", alpha.f = 0.4)
with(summary_df, {
  polygon(
    x      = c(date, rev(date)),
    y      = c(upper, rev(lower)),
...
points(summary_df$date, sen_row)
if (length(red_flag_full) >= 1){
points(summary_df$date, red_flag_full, col="red", pch = 19)
}
